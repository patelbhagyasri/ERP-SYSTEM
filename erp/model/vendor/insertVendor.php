<?php
	...
	if(isset($_POST['vendorDetailsStatus'])){
		...
		if(isset($fullName) && isset($mobile) && isset($address)) {
			...
			$stmt->execute(array('fullName' => $fullName, 'email' => $email, 'mobile' => $mobile, 'phone2' => $phone2, 'address' => $address, 'address2' => $address2, 'city' => $city, 'district' => $district, 'status' => $status));

<?php
	...
	if(isset($_POST['loginUsername'])){
		...
		if(!empty($loginUsername) && !empty($loginUsername)){
			...
			//echo "user and password print !!";
			//echo $loginUsername;
			//echo $loginPassword;
			
			// Check the given credentials
			if($loginUsername=='admin' || $loginUsername=='Transfer' )
			{
			//echo "admin";
			$checkUserSql = 'SELECT * FROM user WHERE username = :username AND password = :password';
			$checkUserStatement = $conn->prepare($checkUserSql);
		 	$checkUserStatement->execute(array('username' => $loginUsername, 'password' => $hashedPassword));
			...
			$checkUserSql = "SELECT customerID,fullName FROM  customer WHERE email = '".$loginUsername."' AND mobile = '".$loginPassword."'";
			$checkUserStatement = $conn->query($checkUserSql);

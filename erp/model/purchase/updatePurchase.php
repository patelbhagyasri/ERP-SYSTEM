...
// Updated script - 2018-05-09
	...
	if(isset($_POST['purchaseDetailsPurchaseID'])){
		...
		if(isset($purchaseDetailsItemNumber) && isset($purchaseDetailsPurchaseDate) && isset($purchaseDetailsQuantity) && isset($purchaseDetailsUnitPrice)){
			...
			$originalPurchaseQuantityStatement->execute(array('purchaseID' => $purchaseDetailsPurchaseID));
			...
			$vendorIDStatement->execute(array('fullName' => $purchaseDetailsVendorName));
			...
			if($originalPurchaseQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $purchaseDetailsItemNumber) {
					...
					$newItemCurrentStockStatement->execute(array('itemNumber' => $purchaseDetailsItemNumber));
					...
					$newItemStockUpdateStatement->execute(array('stock' => $newItemNewStock, 'itemNumber' => $purchaseDetailsItemNumber));
					...
					$previousItemCurrentStockStatement->execute(array('itemNumber' => $originalOrderItemNumber));
					...
					$previousItemStockUpdateStatement->execute(array('stock' => $previousItemNewStock, 'itemNumber' => $originalOrderItemNumber));
					...
					$stockStatement->execute(array('itemNumber' => $purchaseDetailsItemNumber));
					...
					if($stockStatement->rowCount() > 0){
						...
						$updateStockStatement->execute(array('stock' => $newStock, 'itemNumber' => $purchaseDetailsItemNumber));
						...
						$updatePurchaseDetailsStatement = $conn->prepare($updatePurchaseDetailsSql);
						$updatePurchaseDetailsStatement->execute(array('purchaseDate' => $purchaseDetailsPurchaseDate, 'unitPrice' => $purchaseDetailsUnitPrice, 'quantity' => $purchaseDetailsQuantity, 'vendorName' => $purchaseDetailsVendorName, 'vendorID' => $vendorID, 'purchaseID' => $purchaseDetailsPurchaseID));

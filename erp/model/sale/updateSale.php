<?php
	...
	if(isset($_POST['saleDetailsSaleID'])){
		...
		if(isset($saleDetailsItemNumber) && isset($saleDetailsSaleDate) && isset($saleDetailsQuantity) && isset($saleDetailsUnitPrice) && isset($saleDetailsCustomerID)){
			...
			$originalSaleQuantityStatement->execute(array('saleID' => $saleDetailsSaleID));
			...
			$customerIDStatement = $conn->prepare($customerIDsql);
			$customerIDStatement->execute(array('customerID' => $saleDetailsCustomerID));
			...
			if($originalSaleQuantityStatement->rowCount() > 0){
				...
				if($originalOrderItemNumber !== $saleDetailsItemNumber) {
					...
					$newItemCurrentStockStatement->execute(array('itemNumber' => $saleDetailsItemNumber));
					...
					$newItemStockUpdateStatement->execute(array('stock' => $newItemNewStock, 'itemNumber' => $saleDetailsItemNumber));
					...
					$stockStatement->execute(array('itemNumber' => $saleDetailsItemNumber));
					...
					if($stockStatement->rowCount() > 0){
						...
						$updateStockStatement->execute(array('stock' => $newStock, 'itemNumber' => $saleDetailsItemNumber));

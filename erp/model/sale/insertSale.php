<?php
	...
	if(isset($_POST['saleDetailsItemNumber'])){
		...
		if(!empty($itemNumber) && isset($customerID) && isset($saleDate) && isset($quantity) && isset($unitPrice)){
			...
			$stockStatement->execute(array('itemNumber' => $itemNumber));
			if($stockStatement->rowCount() > 0){
				...
				if($currentQuantityInItemsTable <= 0) {
					...
					$customerStatement->execute(array('customerID' => $customerID));
					...
					if($customerStatement->rowCount() > 0){
						...
						$insertSaleStatement->execute(array('itemNumber' => $itemNumber, 'itemName' => $itemName, 'discount' => $discount, 'quantity' => $quantity, 'unitPrice' => $unitPrice, 'customerID' => $customerID, 'customerName' => $customerName, 'saleDate' => $saleDate));
						...
						$stockUpdateStatement->execute(array('stock' => $newQuantity, 'itemNumber' => $itemNumber));
						
						
						// INSERT data to History Table table
						$insertSaleSql = 'INSERT INTO history(itemNumber, itemName, discount, quantity, unitPrice, customerID, customerName, saleDate) VALUES(:itemNumber, :itemName, :discount, :quantity, :unitPrice, :customerID, :customerName, :saleDate)';
						$insertSaleStatement = $conn->prepare($insertSaleSql);
						$insertSaleStatement->execute(array('itemNumber' => $itemNumber, 'itemName' => $itemName, 'discount' => $discount, 'quantity' => $quantity, 'unitPrice' => $unitPrice, 'customerID' => $customerID, 'customerName' => $customerName, 'saleDate' => $saleDate));

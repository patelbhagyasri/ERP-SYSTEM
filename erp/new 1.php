		...
		<h1 class="my-4"></h1>
			<div class="nav flex-column nav-pills" id="v-pills-tab" role="tablist" aria-orientation="vertical">
			  ...
			  <a class="nav-link" id="v-pills-reports-tab" data-toggle="pill" href="#v-pills-reports" role="tab" aria-controls="v-pills-reports" aria-selected="false">Reports</a>
			   <a class="nav-link" id="v-pills-request-tab" data-toggle="pill" href="#v-pills-request" role="tab" aria-controls="v-pills-request" aria-selected="false">New Requests</a>
			</div>
		</div>
		 <div class="col-lg-10">
				...
				<div class="card card-outline-secondary my-4">
				  ...
				  <div class="card-body">										
					<ul class="nav nav-tabs" role="tablist">
						...
						<li class="nav-item">
							<a class="nav-link active" data-toggle="tab" href="#itemNewRequest">New Requests</a>
						</li>
						<li class="nav-item">	
							<a class="nav-link" data-toggle="tab" href="#ApprovedRequest">Approved Requests</a>
						</li>
						<li class="nav-item">
							<a class="nav-link" data-toggle="tab" href="#PendingRequest">Pending Requests</a>
						</li>
						</ul>
						
						
						<div class="tab-content">
						<div id="itemNewRequest" class="container-fluid tab-pane active">
							<br>			
...
if(isset($_REQUEST['unconfirm']))
	...
	$memstatus=1;
	$sql = "UPDATE itemrequest SET status=:status WHERE  id=:aeid";
...
{				?>	
						...
						<button type="submit" name="rejected" class="btn btn-primary">X</button>
                                            ...
                                            <?php if($result->status == 1)
                                                    {?>
                                                    <a href="index.php?confirm=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Un-Confirm the item')">Approved <i class="fa fa-check-circle"></i></a> 
                                                    <?php } else {?>
                                                    <a href="index.php?unconfirm=<?php echo htmlentities($result->id);?>" onclick="return confirm('Do you really want to Confirm the item')">Reject<i class="fa fa-times-circle"></i></a>
                                                    <?php } ?>
											</td>  
											
											<td>
												<a href="edit-user.php?edit=<?php echo $result->id;?>" target="_blank" onclick="return confirm('Do you want to Edit');">Edit <i class="fa fa-pencil-square-o"></i></a>&nbsp;&nbsp;
					...
					<div id="ApprovedRequest" class="container-fluid tab-pane active">
							<br>			
...
<?php
 ...
 $sql = "SELECT * from  itemrequest where status=1";
$query = $conn -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
{				?>	
				  ...
				  <div id="PendingRequest" class="container-fluid tab-pane active">
							<br>			
...
<?php
 ...
 $sql = "SELECT * from  itemrequest where status=0";
$query = $conn -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;

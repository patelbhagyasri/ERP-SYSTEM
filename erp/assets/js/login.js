...
function login(){
	...
	$.ajax({
		...
		success: function(data){
			...
			if(data.indexOf('Redirecting') >= 0 && loginUsername=='admin'){
				window.location = 'index.php';
			}
			else if(data.indexOf('Redirecting') >= 0 && loginUsername=='Transfer'){
				window.location = 'index_transfer.php';
			}
			else if(data.indexOf('Redirecting') >= 0 ){
